...
[`table()`](https://rdrr.io/r/base/table.html), leaves much to be
desired:
...
[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md) is an
approach to tabulating variables that addresses these shortcomings. It’s
part of the janitor package because counting is such a fundamental part
of data cleaning and exploration.

[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md) is
tidyverse-aligned and is primarily built upon the dplyr and tidyr
packages.
...
produces frequency tables using 1, 2, or 3 variables. Under the hood,
[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md) also
attaches a copy of these counts as an attribute of the resulting
data.frame.
...
denominator. And while
[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md) is
built to take a data.frame and column names, you can also produce a
one-way tabyl by calling it directly on a vector:
...
#>   feminine 33.33% (3) 0.00% (0) 44.44%  (4) 0.00% (0) 11.11% (1) 11.11% (1)
#>  masculine 34.62% (9) 3.85% (1) 46.15% (12) 3.85% (1)  3.85% (1)  0.00% (0)
#>     yellow
#>  0.00% (0)
#>  7.69% (2)
```
...
Just as [`table()`](https://rdrr.io/r/base/table.html) accepts three
variables, so does
[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md),
...
apply themselves to each data.frame in the list:
...
#> $feminine
#>            skin_color                    
#>  eye_color       fair     light      none
#>       blue  22.2% (2) 11.1% (1)  0.0% (0)
#>      brown  11.1% (1) 33.3% (3)  0.0% (0)
#>      hazel   0.0% (0) 11.1% (1)  0.0% (0)
#>    unknown   0.0% (0)  0.0% (0) 11.1% (1)
#>      Total  33.3% (3) 55.6% (5) 11.1% (1)
#> 
#> $masculine
#>            skin_color                                                
...
However, if feels more natural to call these with
[`map()`](https://purrr.tidyverse.org/reference/map.html) or
[`lapply()`](https://rdrr.io/r/base/lapply.html), that is still
supported. For instance, `t3 %>% lapply(adorn_percentages)` would
produce the same result as `t3 %>% adorn_percentages`.
...
base R [`table()`](https://rdrr.io/r/base/table.html) object.
...
- **[`adorn_pct_formatting()`](https://sfirke.github.io/janitor/reference/adorn_pct_formatting.md)**:
  Format percentage columns, controlling the number of digits to display
  and whether to append the `%` symbol
- **[`adorn_rounding()`](https://sfirke.github.io/janitor/reference/adorn_rounding.md)**:
  Round a data.frame of numbers (usually the result of
  `adorn_percentages`), either using the base R
  [`round()`](https://rdrr.io/r/base/Round.html) function or using
  janitor’s
  ...
  - e.g., round 10.5 up to 11, consistent with Excel’s tie-breaking
    ...
    - This contrasts with rounding 10.5 down to 10 as in base R’s
      `round(10.5)`.
  - [`adorn_rounding()`](https://sfirke.github.io/janitor/reference/adorn_rounding.md)
    returns columns of class `numeric`, allowing for graphing, sorting,
    etc. It’s a less-aggressive substitute for
    [`adorn_pct_formatting()`](https://sfirke.github.io/janitor/reference/adorn_pct_formatting.md);
    these two functions should not be called together.
- **[`adorn_ns()`](https://sfirke.github.io/janitor/reference/adorn_ns.md)**:
  add Ns to a tabyl. These can be drawn from the tabyl’s underlying
  counts, which are attached to the tabyl as metadata, or they can be
  supplied by the user.
- **[`adorn_title()`](https://sfirke.github.io/janitor/reference/adorn_title.md)**:
  add a title to a tabyl (or other data.frame). Options include putting
  the column title in a new row on top of the data.frame or combining
  the row and column titles in the data.frame’s first name slot.
...
results of calls to
[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md). E.g.,
...
specify columns the same way you would using
...
Now to `adorn_` it. Since this is not the result of a
[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md) call,
it doesn’t have the underlying Ns stored in the `core` attribute, so
...
stored in its `"core"` attribute. Here we retrieve those with
[`attr()`](https://rdrr.io/r/base/attr.html), then apply the base R
function [`format()`](https://rdrr.io/r/base/format.html) to all numeric
columns. Lastly, we append these Ns using
[`adorn_ns()`](https://sfirke.github.io/janitor/reference/adorn_ns.md).
...
you have suggestions related to
[`tabyl()`](https://sfirke.github.io/janitor/reference/tabyl.md) and its
`adorn_` helpers or encounter problems while using them.
